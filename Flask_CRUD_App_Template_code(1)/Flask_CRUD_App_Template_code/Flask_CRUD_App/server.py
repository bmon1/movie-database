...
conn = sqlite3.connect('C:/Users/brand/PycharmProjects/Flask_CRUD_App_Template_code(1)/Flask_CRUD_App_Template_code/CRUDdb.sqlite', check_same_thread=False)
print("opened sqlite")


#df.to_sql("all_movies", conn)
'''
mysql = MySQL()
...
def movies():
    ...
    if 'loggedin' in session:
        ...
        # if the movies table is empty, fill it with the json data
        cursor = conn.cursor()
        cursor.execute('SELECT count(*) FROM movies')
        full = cursor.fetchone()[0]
        if full == 0:
            for k in Movies:
                if not k['cast'] and not k['genres']:
                    cursor.execute('INSERT INTO all_movies (Title, Year, Cast, Genres) VALUES (?, ?, ?, ?)',
                                   (k['title'], k['year'], 'NULL', 'NULL'))

                elif not k['cast']:
                    k['cast'] = 'NULL'
                    for x in k['genres']:
                        cursor.execute('INSERT INTO movies (Title, Year, Cast, Genres) VALUES (?, ?, ?, ?)',
                                       (k['title'], k['year'], 'NULL', x))

                elif not k['genres']:
                    k['genres'] = 'NULL'
                    for x in k['cast']:
                        cursor.execute('INSERT INTO movies (Title, Year, Cast, Genres) VALUES (?, ?, ?, ?)',
                                   (k['title'], k['year'], x, 'NULL'))

                else:
                    for x in k['cast']:
                        cursor.execute('INSERT INTO movies (Title, Year, Cast, Genres) VALUES (?, ?, ?, ?)',
                                   (k['title'], k['year'], x, k['genres'][0]))
                    for x in k['genres']:
                        cursor.execute('INSERT INTO movies (Title, Year, Cast, Genres) VALUES (?, ?, ?, ?)',
                                       (k['title'], k['year'], k['cast'][0], x))
        # User is loggedin show them the home page
        # Get form data
        ...
        genres = request.form.get("form_genres")
        watched = request.form.get("watched")

        # if a movie has been selected split the genres into individual genre
        if genres:
            genres = genres[9:]
            genres = genres[:-1]
        ...
        #if a title has been chosen, enter movie info into watched table
        if title:
            cursor = conn.cursor()
            for eachGenre in genres:
                dateTimeNow = datetime.datetime.now()
                cursor.execute('INSERT INTO watched (username, title, genre, dateTime) VALUES (?, ?, ?, ?)',
                            (session['username'], title, eachGenre, dateTimeNow))
...
def history():
    ...
    if 'loggedin' in session:
        ...
        #find the count of the top searched genre, then select 10 random movies of that genre to be passed to history page
        cursor.execute('SELECT Genre, COUNT(Genre) FROM watched GROUP BY Genre ORDER BY COUNT(Genre) DESC;')
        top_genre = cursor.fetchone()[0]
        count_top_genre = cursor.fetchone()[1]
        cursor.execute('SELECT * FROM movies WHERE Genres = ? ORDER BY random()', (str(top_genre),))
        recommended_list = []
        count = 0
        while count < 10:
            title = cursor.fetchone()[0]
            year = cursor.fetchone()[1]
            recommended_list.append(list([title, year]))
            count += 1

        # find all movies with top genre
        # randomize movies with top genre
        # cut all but the first 10 with top genre then pass list into history.html to render
        # Show the history page with user statistics
        return render_template('history.html', ulist=user_list, top_genre=top_genre,count_top_genre=count_top_genre,
                               recommended_list=recommended_list, index=0)
    # User is not loggedin redirect to login page
    return redirect(url_for('login'))
